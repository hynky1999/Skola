...
while not done:
    ...
    if len(nextByte) == 1:
        ...
        if len(pkt) == 3:  # We got the whole 3 byte packet, start processing it ...
            ...
            xSA4 = pkt[1] | ((pkt[0] & 0x10) << 4)      # Solution of self-assessment assignment 11 for 4th lecture

            xSigned = uint16(pkt[1])                    # Solution of self-assessment assignment 12 for 4th lecture
            if pkt[0] & 0x10 != 0:
                xSigned |= 0xFF00
            xSigned = int16(xSigned);
            
            print(", x=", x, ", x(SA4)=", xSA4, ", x(Signed)=", xSigned, end="")

            y = pkt[2]
            if pkt[0] & 0x20 != 0:
                y |= 0x100

            ySA4 = pkt[2] | ((pkt[0] & 0x20) << 3)      # Solution of self-assessment assignment 11 for 4th lecture

            ySigned = uint16(pkt[2])                    # Solution of self-assessment assignment 12 for 4th lecture
            if pkt[0] & 0x20 != 0:
                ySigned |= 0xFF00
            ySigned = int16(ySigned);
            
            print(", y=", y, ", y(SA4)=", ySA4, ", y(Signed)=", ySigned, end="")
